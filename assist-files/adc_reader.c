...
#define ADC_CHANNEL_I               ADC_CHANNEL_7 // GPIO35
...
#define INACTIVITY_DURATION_MS      30000 // 30 seconds of inactivity to sleep
#define ULP_WAKEUP_THRESHOLD_RAW    2000  // Adjust this raw ADC value to wake up on throttle spike

// --- CALIBRATION CONSTANTS (Tune these in the lab!) ---
// Formula: Real_Value = (ADC_mV * MULTIPLIER) + OFFSET
#define V_MULTIPLIER                (60.0f / 3300.0f) // Example: Voltage divider scaling
#define V_OFFSET                    0.0f
#define I_MULTIPLIER                (1.0f / 50.0f)    // Example: 50mV per Amp sensor
#define I_OFFSET                    -33.0f            // Example: To center 1.65V to 0A
...
{
    // Force SAR ADC power ON - persists into deep sleep when RTC_PERIPH is kept on
    SET_PERI_REG_BITS(SENS_SAR_MEAS_WAIT2_REG, SENS_FORCE_XPD_SAR, SENS_FORCE_XPD_SAR_PU, SENS_FORCE_XPD_SAR_S);
    ...
    // Set attenuation for current channel (Channel 7) to DB_12
    ...
    // Power down amp, clear FSM
    SET_PERI_REG_BITS(SENS_SAR_MEAS_WAIT2_REG, SENS_FORCE_XPD_AMP, SENS_FORCE_XPD_AMP_PD, SENS_FORCE_XPD_AMP_S);
    ...
    SET_PERI_REG_BITS(SENS_SAR_MEAS_WAIT1_REG, SENS_SAR_AMP_WAIT1, 1, SENS_SAR_AMP_WAIT1_S);
    SET_PERI_REG_BITS(SENS_SAR_MEAS_WAIT1_REG, SENS_SAR_AMP_WAIT2, 1, SENS_SAR_AMP_WAIT2_S);
    SET_PERI_REG_BITS(SENS_SAR_MEAS_WAIT2_REG, SENS_SAR_AMP_WAIT3, 1, SENS_SAR_AMP_WAIT3_S);
}
...
static void adc_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        sum_raw_v += raw_v; sum_raw_i += raw_i;
        sum_mv_v += mv_v; sum_mv_i += mv_i;
        sample_count++;

        // Process data at 10Hz
        if (sample_count >= SAMPLES_TO_AVERAGE) {
            ...
            // Apply PCB Scaling Math
            float final_voltage = (avg_mv_v * V_MULTIPLIER) + V_OFFSET;
            float final_current = (avg_mv_i * I_MULTIPLIER) + I_OFFSET;
            float final_power = final_voltage * final_current;

            // Enforce floor to avoid negative noise
            if (final_current < 0.05f && final_current > -0.05f) final_current = 0.0f;
            if (final_voltage < 0.0f) final_voltage = 0.0f;
            ...
            ESP_LOGI(TAG, "V: %.2f V | I: %.2f A | P: %.2f W", final_voltage, final_current, final_power);
            ...
            if (final_current < ACTIVITY_CURRENT_THRESHOLD) {
                ...
                inactive_timer_ms = 0; // Racing activity detected, reset timer
            }

            // Reset accumulators
            sum_raw_v = 0; sum_raw_i = 0;
            sum_mv_v = 0; sum_mv_i = 0;
            sample_count = 0;
        }
