...
static void wifi_init_sta(void)
{
    ...
    initialize_sntp(); // <-- NEW: Start fetching time once Wi-Fi starts
}

static bool send_reading(esp_http_client_handle_t client, sensor_data_t *data)
{
    // 1. Break the timestamp_ms back down into seconds and milliseconds
    struct timeval tv;
    tv.tv_sec = data->timestamp_ms / 1000;
    int ms = data->timestamp_ms % 1000;
    
    // 2. Convert to human readable calendar time (applying the NZ timezone)
    struct tm timeinfo;
    localtime_r(&tv.tv_sec, &timeinfo);
    
    // 3. Format string (Year, Month, Day, Hour, Min, Sec, Milli)
    char time_str[64];
    if (timeinfo.tm_year < (2020 - 1900)) {
        // If Wi-Fi dropped immediately on boot and time hasn't synced from internet yet
        strcpy(time_str, "Waiting for Time Sync");
    } else {
    ...
    // 4. Update JSON to include the timestamp string
    char post_data[300];
    snprintf(post_data, sizeof(post_data),
             "{\"deviceId\":\"esp32-02\",\"timestamp\":\"%s\",\"voltage\":%.2f,\"current\":%.2f,\"power\":%.2f}",
             time_str, data->voltage_v, data->current_a, data->power_w);

    esp_http_client_set_post_field(client, post_data, strlen(post_data));

    esp_err_t err = esp_http_client_perform(client);
    if (err == ESP_OK) {
        // Muting this log slightly so it doesn't spam your terminal as hard
        // ESP_LOGI(TAG, "Sent Data | Time: %s", time_str);
        return true;
    } else {
...
{
    ESP_LOGI(TAG, "Initializing SNTP for Network Time...");
    esp_sntp_setoperatingmode(SNTP_OPMODE_POLL);
    esp_sntp_setservername(0, "pool.ntp.org");
    esp_sntp_init();
    
    // Set Timezone to Auckland, New Zealand (NZST/NZDT)
